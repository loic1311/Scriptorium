...
function brand63(){
  document.title='Scriptorium V6.3 - Academische Onderzoekscoach';
  document.querySelectorAll('.version-pill').forEach(x=>x.textContent='V6.3');
  const s=document.querySelector('.brand small'); if(s)s.textContent='Onderzoekscoach V6.3 · GitHub-ready · authentieke bronnen · training · sync';
  const sub=document.querySelector('.topbar-sub'); if(sub)sub.textContent='Scriptorium V6.3 · web, desktop en mobiel';
}
...
window.init=async function(){
  ...
  if(!document.getElementById('bootNotice')?.classList.contains('bad')) clearBoot();
